...
#   docker build -t avera:cpu -f AvERA.cpu.dockerfile .
# Run it using
#   docker run --rm -it --user "$(id -u):$(id -g)" -v "$PWD:/work" avera:cpu
...
RUN set -eux; \
    ...
    if [ -f Makefile ]; then \
      ...
      if ! grep -q 'Wl,-rpath,/opt/dtfe/DTFE_lib' Makefile; then \
        sed -i 's|^LDFLAGS[[:space:]]*\+=.*|LDFLAGS += $(D_LIB) -Wl,-rpath,/opt/dtfe/DTFE_lib -Wl,-rpath,/opt/voro++/lib|g' Makefile || true; \
      fi; \
    fi
...
# Build AvERA (CPU path; USING_CUDA=NO by default)
...
COPY --from=build /avera/CCLEA /usr/local/bin/avera
